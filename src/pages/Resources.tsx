...
import { Search, Bookmark, BookmarkCheck, ArrowRight } from "lucide-react";
...
export default function Resources() {
  ...
  const filtered = resources.filter((r) => {
    ...
    const matchesFormat = activeFormat === "All" || r.type === activeFormat.toLowerCase().slice(0, -1);
          ...
          {filtered.length > 0 && (
            <div className="flex justify-center pt-10">
              <Button variant="outline" className="rounded-full px-10 font-headline gap-2">
                Load More Resources <ArrowRight className="h-4 w-4" />
              </Button>
            </div>
          )}
        </div>
